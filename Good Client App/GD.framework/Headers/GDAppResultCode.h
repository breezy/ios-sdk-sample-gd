...
typedef enum
{
    ...
    /**< Programmatic activation failed.\ As part of the Programmatic activation authorization
     * processing.\ This code notifies the application
     * that programmatic activation failed due to lack of data connectivity. \ Processing did not succeed this time,
     * but may succeed if another <TT>programmaticAuthorize</TT> call is made when the device data connection is
     * re-established.
     */
    ...
    /**< Programmatic activation failed.\ As part of the Programmatic activation authorization
     * processing.\ This code notifies the application
     * that programmatic activation failed due to incorrect enterprise credentials being passed.
     * \ Processing did not succeed this time, but may succeed if another <TT>programmaticAuthorize</TT> call is made
     * with an updated set of credentials.
     */
    
    GDErrorProgrammaticActivationServerCommsFailed = -603,
    /**< Programmatic activation failed.\ As part of the Programmatic activation authorization
     * processing.\ This code notifies the application
     * that programmatic activation failed due to some enterprise server connectivity failure \ Processing did not succeed this time,
     * but may succeed if another call is made.\ We recommend you implement some exponential back off logic in case this error occurs 
     * multiple times.
     */
    ...
    /**< Programmatic activation failed.\ As part of the Programmatic activation authorization
     * processing.\ This code notifies the application
     * that programmatic activation failed due to some internal unknown failure \ Processing did not succeed this time.
     */
} GDAppResultCode;
