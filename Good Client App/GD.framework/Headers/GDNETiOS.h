...
typedef NSUInteger GDHttpRequest_state_t;
...
/** Standards-based HTTP request, also supporting HTTPS and
 ...
 * Call this function to use the open HTTP request to upload a file.
 * The HTTP request's method can be "PUT" or a custom method.
 ...
 *
 * The file will not be deleted after it is uploaded.
 * Uploading directly from the Good Dynamics secure file system is not
 * supported.
 *
 * \param pathAndFileName <TT>NSString</TT> containing the path (optional) and
 * filename of the file to upload. If path is omitted, the file is read from
...
- (void) setMaxCacheFileSize:(unsigned int) fileSize;
 ...
 *
 * \param fileSize <TT>unsigned int</TT> representing the maximum file size in
 *                 bytes.
 */
 ...
 * \ref GDURLLoadingSystem.
 *
 * \returns <TT>unsigned int</TT> representing the maximum file size in bytes.
 */
 ...
 * initiated by the \link GDiOS::authorize:andVersion:withDelegate:
 * GDiOS authorize\endlink function.
 * The <TT>enableSecure</TT><TT>Communication</TT> function
 * need only be called if access was disabled after authorization, see the
