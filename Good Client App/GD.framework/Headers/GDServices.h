...
/**< Good Inter-Container Communication service request transmission complete callback.
...
/**< Good Inter-Container Communication service request transmission start callback.
 *  */
 ...
 * <TT>GDAppDetail</TT> object returned by the     \link GDiOS::getApplicationDetailsForService:andVersion:andType:         getApplicationDetailsForService:     (GDiOS)\endlink
 * function.
 *
...
+ (BOOL) cancelPendingRequest:(NSString*)requestID toApplication:(NSString*)application;
/**< Cancel a pending request with requestID or all pending requests when requestID is nil .
 *  */
 ...
 * service consumers.
 * \see     \link GDiOS::getApplicationDetailsForService:andVersion:andType:         getApplicationDetailsForService:     (GDiOS)\endlink for the service discovery API.
 ...
 * service discovery.
 * See     \link GDiOS::getApplicationDetailsForService:andVersion:andType:         getApplicationDetailsForService:     (GDiOS)\endlink for details of the service discovery API.
 *
 ...
 *
 * Known issue: The ICC system does not support the sending of empty files.
 * A file of zero length can be attached to a service request or service response,
 * but will not be created in the secure store of the recipient.
 * Where an empty file may be expected as an attachment, the recipient application
 * should check whether the path exists in the secure store before attempting to process.
 * If the path is found not to exist, the application could create an empty file
 * itself, in order to work around this issue.
 * 
